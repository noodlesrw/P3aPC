...
*-*-irix6*)
  # Find out which ABI we are using.
  echo '#line 6025 "configure"' > conftest.$ac_ext
  if { { eval echo "\"\$as_me\":${as_lineno-$LINENO}: \"$ac_compile\""; } >&5
  (eval $ac_compile) 2>&5
  ac_status=$?
  $as_echo "$as_me:${as_lineno-$LINENO}: \$? = $ac_status" >&5
...
if ${lt_cv_prog_compiler_pic_works_CXX+:} false; then :
  ...
  lt_cv_prog_compiler_pic_works_CXX=no
   ...
   -e 's: [^ ]*conftest\.: $lt_compiler_flag&:; t' \
   -e 's:$: $lt_compiler_flag:'`
   (eval echo "\"\$as_me:13994: $lt_compile\"" >&5)
   (eval "$lt_compile" 2>conftest.err)
   ac_status=$?
   cat conftest.err >&5
...
CheckAndroidAudio()
{
    # Check whether --enable-androidaudio was given.
    # Check whether --enable-androidaudio was given.
if test "${enable_androidaudio+set}" = set; then :
  enableval=$enable_androidaudio;
else
  enable_androidaudio=yes
fi


    if test x$enable_audio = xyes -a x$enable_androidaudio = xyes; then
...
if test "x$ac_cv_header_mint_falcon_h" = xyes; then :
...
{ $as_echo "$as_me:${as_lineno-$LINENO}: checking for $ac_word" >&5
$as_echo_n "checking for $ac_word... " >&6; }
if ${ac_cv_path_CACACONFIG+:} false; then :
  $as_echo_n "(cached) " >&6
else
  case $CACACONFIG in
  [\\/]* | ?:[\\/]*)
...
if test "x$ac_cv_header_gem_h" = xyes; then :
  have_gem_hdr=yes
fi
...
if ${ac_cv_lib_gem_appl_init+:} false; then :
  $as_echo_n "(cached) " >&6
else
  ac_check_lib_save_LIBS=$LIBS
LIBS="-lgem  $LIBS"
...
if test "x$ac_cv_lib_gem_appl_init" = xyes; then :
  have_gem_lib=yes
fi
...
CheckAndroidVideo()
{
    # Check whether --enable-video-android was given.
if test "${enable_video_android+set}" = set; then :
  enableval=$enable_video_android;
else
  enable_video_dummy=yes
fi

    if test x$enable_video_android = xyes; then
        $as_echo "#define SDL_VIDEO_DRIVER_ANDROID 1" >>confdefs.h

        SOURCES="$SOURCES $srcdir/src/video/android/*.c"
        have_video=yes
    fi
}
...
if test "x$ac_cv_header_ldg_h" = xyes; then :
  have_ldg_hdr=yes
fi
...
if ${ac_cv_lib_ldg_ldg_open+:} false; then :
  $as_echo_n "(cached) " >&6
else
  ac_check_lib_save_LIBS=$LIBS
LIBS="-lldg -lgem $LIBS"
...
if test "x$ac_cv_lib_ldg_ldg_open" = xyes; then :
  have_ldg_lib=yes
else
  have_ldg_lib=no
fi
