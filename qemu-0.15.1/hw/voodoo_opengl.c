...
void ogl_printInfoLog(GLhandleARB obj)
{
    ...
    if (infologLength > 0)
    {
		infoLog = (char *)qemu_malloc(infologLength);
...
char *strappnd( char *psrc, const char *papn )
{
	unsigned int sL, aL;
	sL=strlen(psrc==NULL?"":psrc);
	aL=strlen(papn);
	if ( psrc==NULL )
		psrc=qemu_malloc( sL+aL+1 );
	else
		psrc=qemu_realloc( psrc, sL=aL+1 );
	strcat(psrc, papn );
	return psrc;
}

void ogl_sh_tex_combine( char **strFShader, const int TMU, const poly_extra_data *extra) {
	voodoo_state *v=(voodoo_state*)extra->state;

	UINT32 TEXMODE     = v->tmu[TMU].reg[textureMode].u;

	if (!TEXMODE_TC_ZERO_OTHER(TEXMODE))
		*strFShader = strappnd( *strFShader, "  tt.rgb = cother.rgb;\n");
	else
		*strFShader = strappnd( *strFShader, "  tt.rgb = vec3(0.0);\n");

	if (!TEXMODE_TCA_ZERO_OTHER(TEXMODE))
		*strFShader = strappnd( *strFShader, "  tt.a = cother.a;\n");
	else
		*strFShader = strappnd( *strFShader, "  tt.a = 0.0;\n");

	if (TEXMODE_TC_SUB_CLOCAL(TEXMODE))
		*strFShader = strappnd( *strFShader, "  tt.rgb -= clocal.rgb;\n");
	if (TEXMODE_TCA_SUB_CLOCAL(TEXMODE))
		*strFShader = strappnd( *strFShader, "  tt.a -= clocal.a;\n");

	switch (TEXMODE_TC_MSELECT(TEXMODE))
	{
	default:
	case 0:
		*strFShader = strappnd( *strFShader, "  blend.rgb = vec3(0.0);\n");
		break;
	case 1:
		*strFShader = strappnd( *strFShader, "  blend.rgb = clocal.rgb;\n");
		break;
	case 2:
		*strFShader = strappnd( *strFShader, "  blend.rgb = vec3(cother.a);\n");
		break;
	case 3:
		*strFShader = strappnd( *strFShader, "  blend.rgb = vec3(clocal.a);\n");
		break;
	case 4:
		// detail //
	case 5:
		// LOD //
		*strFShader = strappnd( *strFShader, "  blend.rgb = vec3(f_lodblend");
		*strFShader = strappnd( *strFShader, TMU==0?"0":"1");
		*strFShader = strappnd( *strFShader, ");\n");
		break;
	}
	...
	default:
	case 0:
		*strFShader = strappnd( *strFShader, "  blend.a = 0.0;\n");
		break;
	case 1:
		*strFShader = strappnd( *strFShader, "  blend.a = clocal.a;\n");
		break;
	case 2:
		*strFShader = strappnd( *strFShader, "  blend.a = cother.a;\n");
		break;
	case 3:
		*strFShader = strappnd( *strFShader, "  blend.a = clocal.a;\n");
		break;
	...
	case 5:
		// LOD //
		*strFShader = strappnd( *strFShader, "  blend.a = f_lodblend");
		*strFShader = strappnd( *strFShader, TMU==0?"0":"1");
		*strFShader = strappnd( *strFShader, ";\n");
		break;
	}
	...
	if (!TEXMODE_TC_REVERSE_BLEND(TEXMODE))
		*strFShader = strappnd( *strFShader, "  blend.rgb = vec3(1.0) - blend.rgb;\n");

	if (!TEXMODE_TCA_REVERSE_BLEND(TEXMODE))
		*strFShader = strappnd( *strFShader, "  blend.a = 1.0 - blend.a;\n");

	*strFShader = strappnd( *strFShader, "  tt *= blend;\n");
	...
	case 1:
		*strFShader = strappnd( *strFShader, "  tt.rgb += clocal.rgb;\n");
		break;
	case 2:
		*strFShader = strappnd( *strFShader, "  tt.rgb += vec3(clocal.a);\n");
		break;
	}
	...
	*strFShader = strappnd( *strFShader, "  clocal = tt;\n");

	if (TEXMODE_TC_INVERT_OUTPUT(TEXMODE))
		*strFShader = strappnd( *strFShader, "  clocal.rgb = vec3(1.0)-clocal.rgb;\n");

	if (TEXMODE_TCA_INVERT_OUTPUT(TEXMODE))
		*strFShader = strappnd( *strFShader, "  clocal.a = 1.0 - clocal.a;\n");
...
void ogl_sh_color_path(std::string *strFShader, const poly_extra_data *extra) {
	...
	{
	case 0:
		*strFShader = strappnd( *strFShader, "  cother = gl_Color;\n");
		break;
	case 1:
		*strFShader = strappnd( *strFShader, "  cother = texel;\n");
		break;
	case 2:
		*strFShader = strappnd( *strFShader, "  cother = color1;\n");
		break;
	default:
		*strFShader = strappnd( *strFShader, "  cother = vec4(0.0);\n");
	}

	// TODO fix chroma key //
	if (FBZMODE_ENABLE_CHROMAKEY(FBZMODE)) {
//		if (!CHROMARANGE_ENABLE(v->reg[chromaRange].u))
			*strFShader = strappnd( *strFShader, "  if (distance (cother.rgb , chromaKey.rgb) < 0.0001) discard;\n");
//		else {
...
//		}
	...
	{
	case 0:
		*strFShader = strappnd( *strFShader, "  cother.a = gl_Color.a;\n");
		break;
	case 1:
		*strFShader = strappnd( *strFShader, "  cother.a = texel.a;\n");
		break;
	case 2:
		*strFShader = strappnd( *strFShader, "  cother.a = color1.a;\n");
		break;
	default:
		*strFShader = strappnd( *strFShader, "  cother.a = 0.0;\n");
		break;
	}
...
//		*strFShader += "  if (cother.a > 0.0) discard;";
	...
	if (ALPHAMODE_ALPHATEST(ALPHAMODE))
		switch (ALPHAMODE_ALPHAFUNCTION(ALPHAMODE)) {
			case 0:
				*strFShader = strappnd( *strFShader, "  discard;\n");
				break;
			case 1:
				*strFShader = strappnd( *strFShader, "  if (cother.a >= alphaRef) discard;\n");
				break;
			case 2:
//				*strFShader += "  if (cother.a != alphaRef) discard;\n";
				*strFShader = strappnd( *strFShader, "  if (distance(cother.a , alphaRef) > 0.0001) discard;\n");
				break;
			case 3:
				*strFShader = strappnd( *strFShader, "  if (cother.a >  alphaRef) discard;\n");
				break;
...
//				*strFShader += "  if (cother.a == alphaRef) discard;\n";
	...
	if (FBZCP_CC_LOCALSELECT_OVERRIDE(FBZCOLORPATH) == 0)
	{
		if (FBZCP_CC_LOCALSELECT(FBZCOLORPATH) == 0)
			*strFShader = strappnd( *strFShader, "  clocal = gl_Color;\n");
		else
			*strFShader = strappnd( *strFShader, "  clocal = color0;\n");
	}
	else
	{
		*strFShader = strappnd( *strFShader, "  if (texel.a < 0.5) {\n"
							"    clocal = gl_Color;\n"
						"  } else {\n"
							"    clocal = color0;\n"
						"  }\n");
	...
	default:
	case 0:
		*strFShader = strappnd( *strFShader, "  clocal.a = gl_Color.a;\n");
		break;
	case 1:
		*strFShader = strappnd( *strFShader, "  clocal.a = color0.a;\n");
		break;
	case 2:
		*strFShader = strappnd( *strFShader, "  clocal.a = gl_Color.a;\n"); // TODO CLAMPED_Z
		break;
	...
	case 3:
		*strFShader = strappnd( *strFShader, "  blend.rgb = vec3(clocal.a);\n");
		break;
	case 4:
		*strFShader = strappnd( *strFShader, "  blend.rgb = vec3(texel.a);\n");
		break;
	case 5:
		// voodoo2 only
		*strFShader = strappnd( *strFShader, "  blend.rgb = texel.rgb;\n");
		break;
	}
	...
	case 3:
		*strFShader = strappnd( *strFShader, "  blend.a = clocal.a;\n");
		break;
	case 4:
		*strFShader = strappnd( *strFShader, "  blend.a = texel.a;\n");
		break;
	}
	...
	*strFShader = strappnd( *strFShader, "  pixel = tt;\n");

	if (FBZCP_CC_INVERT_OUTPUT(FBZCOLORPATH))
		*strFShader = strappnd( *strFShader, "  pixel.rgb = vec3(1.0) - tt.rgb;\n");

	if (FBZCP_CCA_INVERT_OUTPUT(FBZCOLORPATH))
		*strFShader = strappnd( *strFShader, "  pixel.a = 1.0 - tt.a;\n");
}
...
void ogl_sh_fog(std::string *strFShader, const poly_extra_data *extra) {
	...
	*strFShader = strappnd( *strFShader, "  vec4 ff;\n");

	/* constant fog bypasses everything else */
	if (FOGMODE_FOG_CONSTANT(FOGMODE))
		*strFShader = strappnd( *strFShader, "  ff = fogColor;\n");
	...
	/* non-constant fog comes from several sources */
		...
		if (FOGMODE_FOG_ADD(FOGMODE) == 0)
			...
			*strFShader = strappnd( *strFShader, "  ff = vec4(0.0);\n");

		/* if fog_mult is zero, we subtract the incoming color */
		if (FOGMODE_FOG_MULT(FOGMODE) == 0)
			*strFShader = strappnd( *strFShader, "  ff -= pixel;\n");
		...
		*strFShader += "  float fogblend;\n");
		...
		switch (FOGMODE_FOG_ZALPHA(FOGMODE))
		{
			case 0:		/* fog table */
				...
				*strFShader = strappnd( *strFShader, "  fogblend = f_fogblend;\n");
				break;
			case 1:		/* iterated A */
				*strFShader = strappnd( *strFShader, "  fogblend = gl_Color.a;\n");
				break;
			case 2:		/* iterated Z */
				*strFShader = strappnd( *strFShader, "  fogblend = f_fogblend;\n");
				break;
		...
		/* perform the blend */
		*strFShader = strappnd( *strFShader, "  ff *= fogblend;\n");

		/* if fog_mult is 0, we add this to the original color */
		if (FOGMODE_FOG_MULT(FOGMODE) == 0)
			*strFShader = strappnd( *strFShader, "  pixel.rgb += ff.rgb;\n");
...
void ogl_shaders(const poly_extra_data *extra) {
	...
	GLint res;
	char *strVShader=NULL, *strFShader=NULL;
	...
	if (!extra->info->shader_ready) {
		...
		strVShader = strappnd( strVShader,
		...
		const char *szVShader = strVShader;	//strVShader.c_str();
		...
		strFShader = strappnd( strFShader,
			...
			"{\n"
				...
				{
					strFShader = strappnd( strFShader, "  clocal = texture2DProj(tex1,gl_TexCoord[1]);\n" );
					ogl_sh_tex_combine(&strFShader, 1, extra);
					strFShader = strappnd( strFShader, "  cother = clocal;\n" );
					strFShader = strappnd( strFShader, "  texel = clocal;\n" );
				}

				if (texcount >= 1 && v->tmu[0].lodmin < (8 << 8))
				{
					strFShader = strappnd( strFShader, "  clocal = texture2DProj(tex0,gl_TexCoord[0]);\n" );
				...
				strFShader = strappnd( strFShader, "  gl_FragColor = pixel;\n" );

				strFShader = strappnd( strFShader, "}" );

		const char *szFShader = strFShader;	//strFShader.c_str();
